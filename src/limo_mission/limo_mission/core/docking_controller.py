...
class DockingController:
    def __init__(self, node):
        ...
        # Si el movimiento lateral sale al lado incorrecto, cambia esto
        ...
        # Si gira al lado incorrecto, cambia esto
    ...
    def start(self, done_cb=None):
        if self.active:
            self.node.get_logger().warn("Docking ya está activo.")
            return
        ...
        self.node.get_logger().info("Docking iniciado.")
    ...
    def finish(self, success: bool):
        ...
        if success:
            ...
            self.node.get_logger().info("Docking completado con éxito.")
        else:
            self.state = DockingState.FAILED
            self.node.get_logger().error("Docking falló.")
    ...
    def _set_state(self, new_state: DockingState):
        if new_state != self.state:
            self.node.get_logger().info(f"DOCK STATE: {self.state.name} -> {new_state.name}")
    ...
    def control_loop(self):
        ...
        if geom is None:
            ...
            if self.loop_counter % 10 == 0:
                self.node.get_logger().warn('Geometría inválida o cluster inestable.')
            return
